...
The Senior Management team from our client, a supermarket chain, is unclear about how customers are shopping, and how lifestyle choices may affect which food areas customers are buying from, or more interestingly, not buying.

They have asked whether Machine Learning could help segment their customers based on their purchase history with each of the major food categories.  They are hoping this will aid business understanding of their customer base, and enhance the relevance of targetted advertising and customer communications.

<br>
<br>
### Actions <a name="overview-actions"></a>

We first needed to compile the necessary data from tables in the database, namely the *transactions* table and the *product_areas* table.  We joined together the relevant information using Pandas, and then aggregated the transactional data across product areas, from the most recent six months to a customer level.  The final data we used for clustering was, for each customer, the percentage of sales allocated to each product area.

As a starting point, we test & apply k-means clustering for this task.  We need to apply some data pre-processing including most importantly, *feature scaling* to ensure all variables exist on the same scale - a very important consideration for distance-based algorithms such as k-means.
...
Finally, customers in *Cluster 2* spent significant amounts within Dairy, Fruit & Vegetables, but very little in the Meat product area - so similarly, we would make an early hypothesis that these customers are more along the lines of those following a vegetarian diet.
...
Once we have our data separated into distinct clusters, our client needs to understand *what it is* that is driving the separation.  This means the business can understand the customers within each class and the behaviours that make them unique.
